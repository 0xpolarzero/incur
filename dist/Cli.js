...
export function create(nameOrDefinition, definition) {
        ...
        command(nameOrCli, def) {
            if (typeof nameOrCli === 'string') {
                if (def && 'fetch' in def && isFetchSource(def.fetch)) {
                    ...
                    if (def.openapi) {
                        const setOpenapiGroup = (generated) => {
                        ...
                        if (isResolvedOpenapi(def.openapi)) {
                            setOpenapiGroup(Openapi.generateCommandsSync(def.openapi, fetch, {
                            ...
                            pending.push((async () => {
                                ...
                                setOpenapiGroup(await Openapi.generateCommands(spec, fetch, {
                                    basePath: def.basePath,
                                    config: def.openapiConfig,
                                }));
                            })());
    ...
    if (def.mcp)
        toMcpOptions.set(cli, def.mcp);
    if (def.outputPolicy)
        toOutputPolicy.set(cli, def.outputPolicy);
    if (def.sync)
        toSyncOptions.set(cli, def.sync);
    if (def.version !== undefined)
        toVersion.set(cli, def.version);
...
async function serveImpl(name, commands, argv, options = {}) {
    ...
    try {
        ...
        if (human)
            ...
            writeln(Formatter.format({ code: 'UNKNOWN', message }, Formatter.defaultFormat));
        exit(1);
        return;
    }
    ...
    if (skillsIdx !== -1) {
        ...
        if (skillsSub && !sub) {
            ...
            if (human) {
                ...
                writeln(Formatter.format({ code: 'COMMAND_NOT_FOUND', message, cta }, Formatter.defaultFormat));
            exit(1);
            return;
        }
        if (!skillsSub) {
        ...
        try {
            ...
            writeln(Formatter.format({ code: 'SYNC_SKILLS_FAILED', message: err instanceof Error ? err.message : String(err) }, formatExplicit ? formatFlag : Formatter.defaultFormat));
            exit(1);
        }
        return;
    }
    ...
    if (mcpIdx !== -1) {
        ...
        try {
            ...
            writeln(lines.join('\n'));
            if (fullOutput || formatExplicit)
                writeln(Formatter.format({ name, command: result.command, agents: result.agents }, formatExplicit ? formatFlag : Formatter.defaultFormat));
        }
        catch (err) {
            writeln(Formatter.format({ code: 'MCP_ADD_FAILED', message: err instanceof Error ? err.message : String(err) }, formatExplicit ? formatFlag : Formatter.defaultFormat));
            exit(1);
        }
        return;
    }
...
async function fetchImpl(name, commands, req, options = {}) {
    ...
    if (segments[0] === '_incur') {
        const ctx = {
            ...
            middlewares: options.middlewares ?? [],
            name,
            ...(options.rootCommand ? { rootCommand: options.rootCommand } : undefined),
        ...
        if (req.method === 'GET') {
            const resource = (() => {
                if (segments[1] === 'llms')
                    return 'llms';
                if (segments[1] === 'llms-full')
                    return 'llmsFull';
                if (segments[1] === 'schema')
                    return 'schema';
                if (segments[1] === 'help')
                    return 'help';
                if (segments[1] === 'openapi')
                    return 'openapi';
                if (segments[1] === 'skills')
                    return 'skillsIndex';
                if (segments[1] === 'skill')
                    return 'skill';
                if (segments[1] === 'mcp' && segments[2] === 'tools')
                    return 'mcpTools';
                return undefined;
            })();
            if (resource) {
                try {
                    ...
                    return new Response('body' in discovery ? discovery.body : JSON.stringify(discovery.data), {
                        status: 200,
...
function extractBuiltinFlags(argv, options = {}) {
    ...
    let version = false;
    let schema = false;
    let format = Formatter.defaultFormat;
...
function isResolvedOpenapi(source) {
...
export function buildIndexManifest(commands, prefix = []) {
    return {
        version: 'incur.v1',
...
export function buildManifest(commands, prefix = []) {
    return {
        version: 'incur.v1',
    ...
    for (const [name, entry] of commands) {
        ...
        if (isFetchGateway(entry)) {
            ...
            if (schema) {
                cmd.schema = {};
                if (schema.args)
                    cmd.schema.args = schema.args;
                if (schema.env)
                    cmd.schema.env = schema.env;
                if (schema.options)
                    cmd.schema.options = schema.options;
                if (schema.output)
                    cmd.schema.output = schema.output;
            }
...
export function buildCommandSchema(command) {
    ...
    if (output)
        result.output = Schema.toJsonSchema(output);
    return result;
}
