...
export type Result<structured, format> = [format] extends [undefined] ? structured : [format] extends ['json'] ? structured : undefined extends format ? structured | string : string;
...
/** LLM manifest. */
export type LlmsManifest<commands = Client.Commands, scope extends Client.CommandScope<commands> | undefined = undefined> = {
    /** Manifest version. */
    version: string;
    /** Available commands. */
...
/** Full LLM manifest. */
export type LlmsFullManifest<commands = Client.Commands, scope extends Client.CommandScope<commands> | undefined = undefined> = LlmsManifest<commands, scope>;
/** LLM command entry. */
export type LlmsCommand<commands = Client.Commands, scope extends Client.CommandScope<commands> | undefined = undefined> = {
    /** Command name. */
    name: scope extends undefined ? Client.CommandId<commands> : Extract<Client.CommandId<commands>, `${scope}` | `${scope} ${string}`>;
    /** Command description. */
...
export type CommandSchema<_commands = Client.Commands, _command extends string = string> = Record<string, unknown> & {
    /** Args schema. */
...
export type LlmsAction<commands> = {
    <const scope extends Client.CommandScope<commands> | undefined = undefined, const format extends Format | undefined = undefined>(options?: {
        command?: scope | undefined;
        format?: format | undefined;
    } | undefined): Promise<Result<LlmsManifest<commands, scope>, format>>;
};
...
export type LlmsFullAction<commands> = {
    <const scope extends Client.CommandScope<commands> | undefined = undefined, const format extends Format | undefined = undefined>(options?: {
        command?: scope | undefined;
        format?: format | undefined;
    } | undefined): Promise<Result<LlmsFullManifest<commands, scope>, format>>;
};
