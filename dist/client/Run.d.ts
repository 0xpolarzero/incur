...
export type Options<commands, command extends Client.CommandId<commands>> = commands[command] extends {
...
export type Field<name extends string, value> = RequiredKeys<value> extends never ? {
    [key in name]?: value | undefined;
} : {
    [key in name]: value;
};
/** Run input for a command. */
export type Input<commands, command extends Client.CommandId<commands>> = Field<'args', Args<commands, command>> & Field<'options', Options<commands, command>> & (commands[command] extends {
    stream: true;
} ? Omit<Client.Defaults, 'outputTokenCount' | 'outputTokenLimit' | 'outputTokenOffset'> : Client.Defaults);
/** Run input parameter tuple. */
export type InputParameters<commands, command extends Client.CommandId<commands>, input extends Input<commands, command> | undefined> = RequiredKeys<Input<commands, command>> extends never ? [input?: StrictInput<input, Input<commands, command>> | undefined] : [input: StrictInput<input, Input<commands, command>> & Input<commands, command>];
/** Rejects keys outside an expected input shape. */
export type StrictInput<input, shape> = input extends undefined ? undefined : input & {
    [key in Exclude<keyof input, keyof shape>]: never;
} & {
    [key in keyof input & keyof shape]: key extends 'args' | 'options' ? StrictField<input[key], shape[key]> : input[key];
};
/** Rejects keys outside expected `args` or `options` objects. */
export type StrictField<value, shape> = IsUnknown<shape> extends true ? value : NonNullable<shape> extends object ? value & {
    [key in Exclude<keyof value, keyof NonNullable<shape>>]: never;
} : value;
/** Returns true when a type is exactly unknown. */
export type IsUnknown<type> = unknown extends type ? [keyof type] extends [never] ? true : false : false;
...
export type EffectiveRunOutput<output, input, defaults> = EffectiveOutput<output, input extends {
    selection: infer selection;
} ? selection : defaults extends {
    selection: infer selection;
} ? selection : undefined>;
/** Run return type. */
export type Return<commands, command extends Client.CommandId<commands>, input extends Input<commands, command> | undefined, defaults extends Client.Defaults> = commands[command] extends {
    stream: true;
} ? StreamResponse<EffectiveRunOutput<Data<commands, command>, input, defaults>, unknown, commands> : Result<EffectiveRunOutput<Data<commands, command>, input, defaults>, commands>;
/** Run action set. */
export type Actions<commands, defaults extends Client.Defaults> = {
    run<const command extends Client.CommandId<commands>, const input extends Input<commands, command> | undefined = undefined>(command: command, ...input: InputParameters<commands, command, input>): Promise<Return<commands, command, input, defaults>>;
};
...
export type Cta<commands = Client.Commands> = {
    ...
    run<const options extends Client.Defaults | undefined = undefined>(options?: options): Promise<Result<EffectiveOutput<unknown, options extends {
        selection: infer selection;
    } ? selection : undefined>, commands>>;
};
/** Stream response wrapper. */
export type StreamResponse<chunk, finalData = unknown, commands = Client.Commands> = AsyncIterable<chunk> & {
