...
export type ResolvedTransport<transport extends Transport> = ReturnType<transport>['config'] & Omit<ReturnType<transport>, 'config'>;
...
export type Client<commands = Commands, transport extends Transport = Transport, defaults extends Defaults = {}> = Base<transport, defaults> & Run.Actions<commands, defaults> & Resources.Actions<commands> & ([transport] extends [MemoryTransport] ? Local.Methods : {});
/** Options for `Client.create()`. */
export type CreateOptions<transport extends Transport, defaults extends Defaults> = defaults & Defaults & {
...
export type CommandPrefix<command extends string> = command extends `${infer head} ${infer tail}` ? head | `${head} ${CommandPrefix<tail>}` : never;
...
export declare function create<const commands = Commands, const transport extends Transport = Transport, const defaults extends Defaults = {}>(options: CreateOptions<transport, defaults>): Client<commands, transport, defaults>;
