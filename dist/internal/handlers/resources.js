...
export function createResourcesHandler(ctx) {
    return {
        async discover(request) {
            ...
            if (parsed.resource === 'help') {
                ...
                return {
                    ...
                    body: Help.formatRoot(scoped.id, {
                        ...
                        commands: Cli.buildIndexManifest(scoped.commands, []).commands.map(({ name, description }) => ({
                            name,
            ...
            if (format === 'md') {
                ...
                const entries = Cli.collectSkillCommands(scoped.commands, scoped.prefix, groups, scoped.rootCommand);
...
function openapi(ctx) {
    const cli = { name: ctx.name, description: ctx.description };
    Cli.toCommands.set(cli, ctx.commands);
    if (ctx.rootCommand)
        Cli.toRootDefinition.set(cli, ctx.rootCommand);
